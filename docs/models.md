...
- При удалении месторождения автоматически удаляются все его скважины,
  а вместе со скважинами — все связанные с ними заявки на обслуживание.
- При удалении пользователя его заявки **не удаляются**: поле `author_id`
  становится `NULL`, чтобы сохранить историю обращений. В интерфейсе
  такие заявки можно отображать как «автор удалён».
- Это решение зафиксировано на уровне внешних ключей в миграции, поэтому
  работает одинаково при любом способе удаления (через API, ORM или SQL).
